...
const renderDisplayRows = (rows, options = {}) => {
	...
	return rows.map((row) => {
		return `${indent}${style$1(t, "muted", padEnd(row.label, labelWidth))}  ${highlightAislop(row.value, t, row.valueToken)}`;
	});
};
...
var AgentTui = class {
	...
	ensureMounted() {
		...
		this.handle = import("./mount-DAREm7si.js").then((m) => m.mountAgentTui(this.store));
	}
	setActions(actions) {
...
const textFromContent = (content) => {
	...
	return asString$2(content.map((item) => {
...
const renderCurrentPreference = (root) => {
	...
	const rows = [
		...
		{
			label: "Preference",
			value: agentProviderPreferencePath(root)
		},
...
const resolveExtendsRef = (ref, fromDir) => {
	...
	if (ref.startsWith("./") || ref.startsWith("../")) return path.resolve(fromDir, ref);
...
const loadConfigChainInner = (configPath, state, depth) => {
	...
	const parents = refs.map((ref) => {
		return loadConfigChainInner(resolveExtendsRef(ref, fromDir), nextState, depth + 1);
	});
...
const ALLOWED_OVERRIDE = new Set([
	"npm",
	"pnpm",
	"yarn",
	"bun",
...
const updateAgentMonitorRecord = (root, id, update) => {
	const record = readAgentMonitorRecord(agentMonitorPath(root, id));
...
const agentMonitorStopCommand = async (monitor, options) => {
	try {
		const stopped = await stopAgentMonitor(await resolveAgentGitRoot(options.root), monitor, { force: options.force });
		process.stdout.write(`${[
...
const displayPath = (base, target) => isInside(base, target) ? path.relative(base, target) || "." : target;
...
const resolveAgentApplyTarget = (input) => {
	...
	const targetRoot = asString(input.events.find((event) => event.type === "session.started" || event.type === "session.queued")?.root) ?? input.root;
	const worktreePath = asString([...input.events].reverse().find((event) => event.type === "worktree.prepared")?.path);
...
const agentApplyCommand = async (session, options) => {
	try {
		...
		const target = resolveAgentApplyTarget({
			root,
			sessionPath,
			events: readAgentSessionEvents(sessionPath)
		});
...
const selectedCount = (events) => {
	return asNumber(events.find((item) => item.type === "findings.selected")?.count);
};
const verifiedDiagnostics = (events) => {
	return asNumber(asObject(events.find((item) => item.type === "diff.verified")?.scan)?.diagnostics);
};
...
const agentStopCommand = async (session, options) => {
	try {
		renderAgentStopResult(await stopAgentSession(await resolveAgentGitRoot(options.root), session, { force: options.force }));
	} catch (error) {
		log.error(error instanceof Error ? error.message : String(error));
		process.exitCode = 1;
	}
...
const isExcludedPath = (filePath) => {
	...
	return EXCLUDED_DIRS.some((dir) => normalized === dir || normalized.startsWith(`${dir}/`) || normalized.includes(`/${dir}/`));
};
const isExcludedFromScan = (relativePath) => isExcludedPath(relativePath) || isGeneratedArtifactFile(relativePath);
...
const filterProjectFiles = (rootDirectory, files, extraExtensions = [], exclude = [], include = []) => {
	...
	return normalizedFiles.filter(({ absolutePath, relativePath }) => {
		if (!isSafeRegularProjectFile(rootDirectory, absolutePath) || !isWithinProject(relativePath) || isExcludedPath(relativePath) || isTestFile$2(relativePath) || isGeneratedArtifactFile(relativePath) || ignoredPaths.has(relativePath)) return false;
...
const filterExplicitFiles = (rootDirectory, files, extraExtensions = []) => {
	...
	}).filter(({ absolutePath, relativePath }) => isWithinProject(relativePath) && isSafeRegularProjectFile(rootDirectory, absolutePath) && hasAllowedExtension(relativePath, extraSet)).map(({ absolutePath }) => absolutePath);
};
...
const JS_TS_EXTENSIONS$1 = new Set([
	".ts",
	".tsx",
	".js",
	".jsx",
...
const detectPythonPrintOnlyExceptions = (content, relativePath) => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (!match) continue;
		if (!isPythonPrintOnlyBody(extractIndentedBlock(lines, i, match[1].length))) continue;
		diagnostics.push({
			filePath: relativePath,
			engine: "ai-slop",
...
const buildViteAliasMatcher = (key) => {
...
const collectAliasMatchersFromConfig = (configPath, matchers) => {
	const opts = readJsoncFile(configPath)?.compilerOptions;
...
const isLocalAliasReplacement = (rawValue) => {
...
const collectViteObjectAliases = (block, matchers) => {
...
const ARRAY_ALIAS_ENTRY_RE = /\{\s*find\s*:\s*(["'])([^"']+)\1[\s\S]*?replacement\s*:\s*([^,\n}]+)[\s\S]*?\}/g;
const collectViteArrayAliases = (block, matchers) => {
...
const collectViteAliasesFromConfig = (configPath, matchers) => {
	let content;
	try {
	...
	for (const match of content.matchAll(/\b(?:alias|aliases)\s*[:=]\s*(?:\x7b|\[)/g)) {
		const openIndex = match.index + match[0].length - 1;
		const block = findBalancedBlock(content, openIndex);
		if (!block) continue;
		if (block.charCodeAt(0) === 123) collectViteObjectAliases(block, matchers);
		else collectViteArrayAliases(block, matchers);
	}
};
...
const collectNestedPackageRootDirs = (rootDir) => {
	...
	const walk = (dir, depth) => {
		...
		for (const entry of entries) {
			...
			else if (entry.name === "package.json" && depth > 0) roots.add(dir);
		}
	};
	walk(rootDir, 0);
	return [...roots];
};
const collectPackageRootDirs = (rootDir, workspaceDirs) => {
	return [...new Set([
		rootDir,
		...workspaceDirs,
...
const collectPackageJsonImportMatchers = (pkgPath, matchers) => {
	const pkg = readJsoncFile(pkgPath);
	if (!pkg || typeof pkg !== "object") return;
	const imports = pkg.imports;
	if (!imports || typeof imports !== "object") return;
	for (const key of Object.keys(imports)) matchers.push(buildAliasMatcher(key));
};
const collectTsPathAliases = (rootDir, workspaceDirs) => {
	const matchers = [];
	const dirs = collectPackageRootDirs(rootDir, workspaceDirs);
	for (const dir of dirs) {
		for (const fname of TS_CONFIG_FILES) collectAliasMatchersFromConfig(path.join(dir, fname), matchers);
		for (const fname of VITE_ALIAS_FILES) collectViteAliasesFromConfig(path.join(dir, fname), matchers);
...
const collectNestedScopes = (rootDir) => {
	...
	const walk = (dir, depth) => {
		...
		const hasManifest = entries.some((entry) => entry.isFile() && (entry.name === "pyproject.toml" || entry.name === "requirements.txt" || entry.name === "Pipfile"));
...
const collectJsScope = (directory) => {
...
const pythonDepsForFile = (manifest, filePath, rootDirectory) => {
	...
	const nestedScope = manifest.pyScopes.filter((scope) => scope.directory !== rootDirectory && scope.hasPyManifest && isWithinDirectory(filePath, scope.directory)).sort((a, b) => b.directory.length - a.directory.length)[0];
...
const isWaspProjectDirectory = (directory) => {
	try {
		...
		const pkg = readJson$1(path.join(directory, "package.json"));
		if (!pkg) return false;
...
const isJsVirtualModule = (spec, jsDeps, filePath, rootDirectory) => {
...
const extractJsImports = (content) => {
	...
	for (let i = 0; i < lines.length; i++) {
		const line = lines[i];
		for (let j = 0; j < line.length; j++) if (line[j] === "`" && (j === 0 || line[j - 1] !== "\\")) inTemplateLiteral = !inTemplateLiteral;
...
const checkJsImport = (rawSpec, jsDeps, tsAliasMatchers, filePath, rootDirectory) => {
...
const detectHallucinatedImports = async (context) => {
	...
	for (const filePath of files) {
		...
		for (const { spec, line } of imports) {
			const hallucinated = isJs ? checkJsImport(spec, jsDeps ?? manifest.jsDeps, tsAliasMatchers, filePath, context.rootDirectory) : checkPyImport(spec, pyDeps ?? manifest.pyDeps);
...
const findBundledKnipBin = () => {
	try {
		const knipRoot = findPackageRoot(requireFromAislop.resolve("knip"), "knip");
...
const runKnip = async (rootDirectory) => {
	const knipRuntime = findKnipRuntime(rootDirectory, findMonorepoRoot(rootDirectory));
...
const resolveOxlintBinary = () => {
	try {
		const oxlintMainPath = esmRequire$2.resolve("oxlint");
...
const lintEngine = {
	...
	async run(context) {
		...
		if (languages.includes("typescript") || languages.includes("javascript")) {
			...
			if (context.config.lint.typecheck) promises.push(Promise.resolve().then(() => typecheck_exports).then((mod) => mod.runTypecheck(context)));
		}
		if (context.frameworks.includes("expo") && context.config.lint.expoDoctor) promises.push(Promise.resolve().then(() => expo_doctor_exports).then((mod) => mod.runExpoDoctor(context)));
	...
	try {
		...
		for (const [packageName, advisories] of Object.entries(parsed)) {
			...
			for (const advisory of advisories) {
				...
				upsertVuln(bucket, packageName, (record.severity ?? "moderate").toLowerCase(), record.title ?? record.vulnerable_versions ?? record.url ?? "");
			}
		}
...
const NON_PRODUCTION_CREDENTIAL_EXACT = new Set([
	"pass",
	"password",
	"pw",
	"mostest"
...
const resolveEffectiveFileCount = (diagnostics, sourceFileCount) => {
	...
	if (typeof sourceFileCount === "number") throw new Error("sourceFileCount must be greater than 0 when diagnostics are present.");
...
const renderTeamCta = (deps = {}) => {
	return `\n ${style$1(deps.theme ?? theme, "muted", `→ Make this your team's standard. Gate every PR free at ${terminalLink("https://scanaislop.com")}`)}\n`;
};
...
const buildScanRender = (input) => {
	...
	const cta = input.printBrand === false ? "" : errors + warnings > 0 ? renderTeamCta(deps) : renderStarCta(deps);
...
const runScanBody = async (resolvedDir, config, options, projectInfo) => {
	...
	if (options.staged) {
		...
		if (!machineOutput) process.stdout.write(renderScopeRow(`${files.length} staged file(s)`));
		...
		if (!machineOutput) process.stdout.write(renderScopeRow(`${files.length} file(s) after exclusions`));
	}
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-TZGSFagb.js");
		console.log(JSON.stringify(buildSarifLog(results), null, 2));
		return completion;
	}
	if (options.json) {
		const { buildJsonOutput } = await import("./json-BsFU0B0W.js");
...
const resolveTrustedTscPath = () => {
	try {
		return esmRequire$1.resolve("typescript/lib/tsc.js");
	} catch {
		return null;
	}
};
...
const runFormattingStep = async (deps) => {
	...
	if (deps.projectInfo.languages.includes("ruby") && deps.projectInfo.installedTools.rubocop) {
		if (!skipUnsafeSafeFormatter(deps, "ruby")) await deps.runStep("Formatting (ruby)", () => runGenericFormatter(deps.context, "ruby"), () => fixGenericFormatter(deps.resolvedDir, "ruby"));
	...
	if (deps.projectInfo.languages.includes("php") && deps.projectInfo.installedTools["php-cs-fixer"]) {
		if (!skipUnsafeSafeFormatter(deps, "php")) await deps.runStep("Formatting (php)", () => runGenericFormatter(deps.context, "php"), () => fixGenericFormatter(deps.resolvedDir, "php"));
...
const runForceSteps = async (deps) => {
	...
	if (deps.projectInfo.frameworks.includes("expo") && deps.config.lint.expoDoctor) await deps.runStep("Expo dependency alignment", () => runExpoDoctor(deps.context), () => fixExpoDependencies(deps.context, railUpdate));
};
