...
const maybeNotifyUpdate = async (now = Date.now()) => {
	...
	if (cache && isOutdated("0.14.1", cache.latest)) process.stderr.write(formatUpdateNotice(APP_VERSION, cache.latest));
...
const renderCommandReference = (input = {}) => {
	const version = input.version ?? "0.14.1";
