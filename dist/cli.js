...
var AgentTui = class {
	...
	ensureMounted() {
		...
		this.handle = import("./mount-ChJj-O37.js").then((m) => m.mountAgentTui(this.store));
	}
	setActions(actions) {
...
const scanCommand = async (directory, config, options) => {
	...
	const projectInfo = {
		...await discoverProject(resolvedDir, excludePatterns, { includePatterns: config.include }),
...
const runScanBody = async (resolvedDir, config, options, projectInfo, scanScope) => {
	...
	const { results, suppressedCount } = applySuppressions((await runEnginesWithProgress({
		rootDirectory: resolvedDir,
		languages: projectInfo.languages,
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-CoGPn-4L.js");
...
const renderCommandReference = (input = {}) => {
	const version = input.version ?? "0.14.0";
...
const maybeNotifyUpdate = async (now = Date.now()) => {
	...
	if (cache && isOutdated("0.14.0", cache.latest)) process.stderr.write(formatUpdateNotice(APP_VERSION, cache.latest));
