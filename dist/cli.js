...
const maybeNotifyUpdate = async (now = Date.now()) => {
	...
	if (cache && isOutdated("0.15.0", cache.latest)) process.stderr.write(formatUpdateNotice(APP_VERSION, cache.latest));
...
const renderCommandReference = (input = {}) => {
	const version = input.version ?? "0.15.0";
...
const runLintSteps = async (deps) => {
	...
	if (deps.projectInfo.languages.includes("ruby") && deps.projectInfo.installedTools.rubocop) await deps.runStep("Lint fixes (ruby)", () => runGenericLinter(deps.context, "ruby"), () => fixRubyLint(deps.resolvedDir));
