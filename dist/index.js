import { t as APP_VERSION } from "./version-CVfOxP_O.js";
...
const scanFileForConfigLiterals = (content, relativePath, ext) => {
	...
	return lines.flatMap((line, index) => scanLineForConfigLiterals(line, relativePath, ext, index + 1, docstringLines?.has(index) ?? false));
};
...
const runScanBody = async (resolvedDir, config, options, projectInfo, scanScope, dependencyAuditLanguages) => {
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-B2tJvi9V.js");
		console.log(JSON.stringify(buildSarifLog(results), null, 2));
		return completion;
	}
	if (options.json) {
		const { buildJsonOutput } = await import("./json-DKuTrkDa.js");
