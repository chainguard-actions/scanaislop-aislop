...
import { t as resolveTrustedTscPath } from "./typecheck-Bs5XKPFr.js";
import { a as getEngineLabel, i as ENGINE_INFO, r as scoreImpactForRule, t as summarizeFindingAssessments } from "./finding-assessment-fSnkh6zi.js";
import { t as APP_VERSION } from "./version-BqVqHF5s.js";
import { r as runGenericLinter, t as fixRubyLint } from "./generic-B_4NojVk.js";
import { n as runExpoDoctor } from "./expo-doctor-C4qYm9if.js";
...
const lintEngine = {
	...
	async run(context) {
		...
		if (languages.includes("typescript") || languages.includes("javascript")) {
			...
			if (context.config.lint.typecheck) promises.push(import("./typecheck-Bs5XKPFr.js").then((n) => n.n).then((mod) => mod.runTypecheck(context)));
		}
		if (context.frameworks.includes("expo") && context.config.lint.expoDoctor) promises.push(import("./expo-doctor-C4qYm9if.js").then((n) => n.t).then((mod) => mod.runExpoDoctor(context)));
...
const runLintSteps = async (deps) => {
	...
	if (deps.projectInfo.languages.includes("ruby") && deps.projectInfo.installedTools.rubocop) await deps.runStep("Lint fixes (ruby)", () => import("./generic-B_4NojVk.js").then((n) => n.n).then((mod) => mod.runGenericLinter(deps.context, "ruby")), () => fixRubyLint(deps.resolvedDir));
...
const runScanBody = async (resolvedDir, config, options, projectInfo) => {
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-CxKV25pm.js");
		console.log(JSON.stringify(buildSarifLog(results), null, 2));
		return completion;
	}
	if (options.json) {
		const { buildJsonOutput } = await import("./json-BCq0duCy.js");
