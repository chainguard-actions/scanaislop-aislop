import { n as relativePosix, r as toPosix, t as projectRelativePosix } from "./paths-BNrMTGml.js";
import { a as warnSubprocessFailure, i as runSubprocess, n as isMissingToolError, r as isToolInstalled, t as chunkFilePaths } from "./subprocess-C04FPuWA.js";
import { t as APP_VERSION } from "./version-CEQz8Pyx.js";
import { a as getEngineLabel, i as ENGINE_INFO, r as scoreImpactForRule, t as summarizeFindingAssessments } from "./finding-assessment-M5fUTE-C.js";
...
const reportPathFor = (rootDirectory) => path.join(rootDirectory, REPORT_FILENAME);
...
const checkTarget = async (context, target) => {
...
const parseJsAudit = (output, source) => {
	...
	try {
		...
		if (isRecord(vulnerabilities)) return parseModernVulnerabilities(vulnerabilities, source);
		return [];
	} catch {
		return [];
	}
...
const runScanBody = async (resolvedDir, config, options, projectInfo, scanScope, dependencyAuditLanguages) => {
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-BhGdt4Bu.js");
		console.log(JSON.stringify(buildSarifLog(results), null, 2));
		return completion;
	}
	if (options.json) {
		const { buildJsonOutput } = await import("./json-DxmocyMg.js");
