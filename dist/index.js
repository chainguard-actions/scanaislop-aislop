import { r as readJsoncFile, t as resolveTrustedTscPath } from "./typecheck-Dt2dfKTC.js";
import { n as runSubprocess, t as isToolInstalled } from "./subprocess-CQUJDGgn.js";
import { a as getEngineLabel, i as ENGINE_INFO, r as scoreImpactForRule, t as summarizeFindingAssessments } from "./finding-assessment-CuVPw5qF.js";
import { t as APP_VERSION } from "./version-CEb-cH9v.js";
import { r as runGenericLinter, t as fixRubyLint } from "./generic-qUQN59G9.js";
import { n as runExpoDoctor } from "./expo-doctor-CrbvRi75.js";
...
const renderDisplayRows = (rows, options = {}) => {
	...
	return rows.map((row) => {
		return `${indent}${style(t, "muted", padEnd(row.label, labelWidth))}  ${highlightAislop(row.value, t, row.valueToken)}`;
	});
};
...
const lintEngine = {
	...
	async run(context) {
		...
		if (languages.includes("typescript") || languages.includes("javascript")) {
			...
			if (context.config.lint.typecheck) promises.push(import("./typecheck-Dt2dfKTC.js").then((n) => n.n).then((mod) => mod.runTypecheck(context)));
		}
		if (context.frameworks.includes("expo") && context.config.lint.expoDoctor) promises.push(import("./expo-doctor-CrbvRi75.js").then((n) => n.t).then((mod) => mod.runExpoDoctor(context)));
...
const runLintSteps = async (deps) => {
	...
	if (deps.projectInfo.languages.includes("ruby") && deps.projectInfo.installedTools.rubocop) await deps.runStep("Lint fixes (ruby)", () => import("./generic-qUQN59G9.js").then((n) => n.n).then((mod) => mod.runGenericLinter(deps.context, "ruby")), () => fixRubyLint(deps.resolvedDir));
...
const renderTeamCta = (deps = {}) => {
	return `\n ${style(deps.theme ?? theme, "muted", `→ Make this your team's standard. Gate every PR free at ${terminalLink("https://scanaislop.com")}`)}\n`;
};
...
const runScanBody = async (resolvedDir, config, options, projectInfo) => {
	...
	if (options.sarif) {
		const { buildSarifLog } = await import("./sarif-C1MEosxw.js");
		console.log(JSON.stringify(buildSarifLog(results), null, 2));
		return completion;
	}
	if (options.json) {
		const { buildJsonOutput } = await import("./json-CGjiWmRU.js");
